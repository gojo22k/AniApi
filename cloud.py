...
def fetch_folder_data(url, cloud_name):
    ...
    try:
        ...
        # Handle Aniflix format
        if cloud_name == 'Aniflix':
            if not data.get('success') or 'folders' not in data:
                return []
            
            folder_data = []
            for platform in data['folders']:
                folder_name = platform.get('name', '')
                folder_id = str(platform.get('folderId', ''))
        ...
        print(f"Error fetching from {cloud_name}: {e}")
        return []
